...
Нужно создать вертуальную машину на ОС debian, установить на ней пакет инструентов LAMP а также настроить подключение к wordpress и phpMyAdmin
...
Команда ```wget http://ссылка``` позволяет скачать данные по протаколу http. В данном случае происходит скачивание архивов.

### Зачем необходимо создавать для каждого сайта свою базу и своего пользователя?

Создание отдельного пользователя и отдельной базы данных для каждого сайта необходима для безопасновти. В случае взлома базы данных одного сайта, данные друго не будут затронуты. Также, это позволяет удобней админестрировать сайты, поскольку пересечения данных с разных сайтов в одной БД могут вызывать трудности для определения необходимых для поиска данных.

### Как поменять доступ к системе управления БД на порт 1234?

В файлах конфиграций, порт определяется в теге ```<VirtualHost *:80>```, таким образом, если мы заменим 80 на 1234, то система будет ждать данных из этого порта.

### Какие преимущества, с вашей точки зрения, даёт виртуализация?

Возможность запускать несколько полностью изолированных систем на одном компьютере. Лёгкое развёртывание систем за счёт виртуальных дисков, однажды настроенная система, может быть установленна множество раз без повторной перенастройки.

### Для чего необходимо устанавливать время / временную зону на сервере?

Логирование операций и ошибок, всегда требует точного времени системы. Для генерации сертификатов для аунтификации и проверки этих сертификатов нужно правильное время.

### Сколько места занимает установленная вами ОС (виртуальный диск) на хостовой машине?

![размер debian на хостовой машине](images/Screenshot_8.png)

### Какие есть рекомендации по разбиению диска для серверов? Почему рекомендуется так разбивать диск?

- /root - раздел для операционной системы
- /home - файлы пользователей
- /var - логи, веб-сайты, базы данных, конфигурации
- /tmp - содержит временные файлы
- /swap - поддержка памяти если закончилась ОЗУ

### Выводы

В ходе работы была развернута виртуальная машина на базе Debian с использованием QEMU, установлен и настроен стек LAMP Были успешно подключены и протестированы WordPress и phpMyAdmin, настроены виртуальные хосты и база данных. Работа позволила получить практические навыки настройки серверного окружения, администрирования БД и конфигурирования веб-сервера в изолированной среде.